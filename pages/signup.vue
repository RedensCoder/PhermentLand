...
const signup = async () => {
  if (nick.value.trim() === "" || mail.value.trim() === "" || pass.value.trim() === "" || subPass.value.trim() === "") {
  ...
  if (pass.value.trim() !== subPass.value.trim()) {
    alert("Пароли не совпадают!")
  } else {
    await PAY.Signup(nick.value.trim(), mail.value.trim(), pass.value.trim());
...
<template>
  ...
  <div class="main">
    ...
    <div class="form">
      ...
      <label>Почта</label>
      <input v-model="mail" type="email" placeholder="Введите вашу почту!">
      <label>Пароль</label>
      <input v-model="pass" type="password" placeholder="Введите пароль!">
      <label>Подтверждение пароля</label>
      <input v-model="subPass" type="password" placeholder="Подтвердите пароль!">
      <p>Уже есть аккаунт? <NuxtLink to="/signin">Авторизация</NuxtLink></p>
